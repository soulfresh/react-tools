...
<Canvas>
  <Story name="Tooltip">
  <p>This is a <ExampleTooltip content="Cool 😎" onOpen={e => console.log('OPEN', e)} onClose={e => console.log('CLOSE', e)}><b>Tooltip Example</b></ExampleTooltip> within
  ...
  <Story name="Controlled">
    <Column>
      {() => {
        ...
        React.useEffect(() => console.log('ref?', ref));
        return (
          <ExampleTooltip
            isOpen={isOpen}
            onOpen={() => (console.log('OPEN'), setIsOpen(true))}
            onClose={() => (console.log('CLOSE'), setIsOpen(false))}
            content="Cool 😎"
          >
            <span ref={ref}>Tooltip Example</span>
          </ExampleTooltip>
        );
      }}
    </Column>
